...
describe('enqueue', () => {
  ...
  it('adds equal priority first-in/first-out', () => {
    const queue = new PriorityQueue<string>();
    queue.enqueue('c', 20);
    queue.enqueue('b', 18);
    queue.enqueue('a', 15);
    ...
    expect(queue.nodes[0]).toEqual({ data: 'c', priority: 20 });
    expect(queue.nodes[1]).toEqual({ data: 'b', priority: 18 });
    expect(queue.nodes[2]).toEqual({ data: 'b2', priority: 18 });
    expect(queue.nodes[3]).toEqual({ data: 'a', priority: 15 });
    expect(queue.nodes[4]).toEqual({ data: 'a2', priority: 15 });
    expect(queue.nodes[5]).toEqual({ data: 'a3', priority: 15 });
...
describe('dequeue', () => {
  ...
  it('removes and returns equal priority first-in/first-out', () => {
    const queue = new PriorityQueue<string>();
    queue.enqueue('c', 20);
    queue.enqueue('b', 18);
    queue.enqueue('a', 15);
