...
export function Todos() {
  ...
  useEffect(() => {
    async function read() {
      try {
        const response = await fetch('/api/todos'); //do not put the full path
  ...
  async function toggleCompleted(todo: Todo) {
    try {
      const result = await fetch(`api/todos/${todo.todoId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!result.ok) throw new Error('create Error');
      const data = (await result.json()) as Todo;
      const newTodos = todos.map((t) => (t.todoId === todo.todoId ? data : t));
      setTodos(newTodos);
    } catch (err) {
      console.error(err);
      alert('Error');
